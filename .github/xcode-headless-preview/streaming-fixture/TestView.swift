    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { context in
            ...
            ZStack {
                ...
                VStack(spacing: 34) {
                    Text("LIVE PREVIEW").font(.system(size: 44, weight: .black, design: .rounded)).foregroundStyle(.yellow)
                    ...
                    HStack(spacing: 22) {
                        ForEach(0..<3) { index in
                            Circle().fill(index == Int(phase * 2).quotientAndRemainder(dividingBy: 3).remainder ? .cyan : .white.opacity(0.18)).frame(width: 54, height: 54)
                        }
                    }
...
#Preview("Live JPEG backpressure stream") { TestView() }
